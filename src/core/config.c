...
void cfg_init(int argc, char *argv[]) {
    ...
    while ((opt = getopt_long(argc, argv, "hkmgvV", long_options, NULL)) != -1) {
        switch (opt) {
            case 'h':  break;
            case 'k':  settings.kib_format = true; break;
            case 'm':  settings.mib_format = true; break;
            case 'g':  settings.gib_format = true; break;
            case 'v':  settings.verbose = true; break;
            case 1001: settings.color = false; break;
            case 1002: ui_print_help(argv[0]); exit(EXIT_SUCCESS);
